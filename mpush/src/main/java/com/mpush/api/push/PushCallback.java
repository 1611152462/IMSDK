package com.mpush.api.push;

//import com.mpush.api.router.ClientLocation;

import android.annotation.TargetApi;
import android.os.Build;

/**
 * Created by ohun on 2015/12/30.
 *
 * @author ohun@live.cn
 */
public interface PushCallback {

    default void onResult(PushResult result) {
        switch (result.resultCode) {
            case PushResult.CODE_SUCCESS:

                onSuccess(result.userId);
                break;
            case PushResult.CODE_FAILURE:
                onFailure(result.userId);
                break;
            case PushResult.CODE_OFFLINE:
                onOffline(result.userId);
                break;
            case PushResult.CODE_TIMEOUT:
                onTimeout(result.userId);
                break;
        }
    }

//    /**
//     * 推送成功, 指定用户推送时重写此方法
//     *
//     * @param userId   成功的用户, 如果是广播, 值为空
//     * @param location 用户所在机器, 如果是广播, 值为空
//     */
////    default void onSuccess(String userId, ClientLocation location) {
////    }

    default void onSuccess(String userId) {
    }

    /**
     * 推送失败
     *
     * @param userId   推送用户
     */
    default void onFailure(String userId) {
    }

    /**
     * 推送用户不在线
     *
     * @param userId   推送用户
     */
    default void onOffline(String userId) {
    }

    /**
     * 推送超时
     *
     * @param userId   推送用户
     */
    default void onTimeout(String userId) {
    }
}