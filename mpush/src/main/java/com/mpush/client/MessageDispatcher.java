...
public final class MessageDispatcher implements PacketReceiver {
    ...
    public MessageDispatcher() {
        ...
        register(Command.RELATION, new RelationMessageHandler());
        register(Command.GROUP, new GroupMessageHandler());
    ...
    public void onReceive(final Packet packet, final Connection connection) {
        Log.e("MPushEngine","接收到的packag:" + packet.toString());
    ...
    private void doAckResponse(Packet packet) {
        ...
        Log.e("dddddd",(null != task)+"======"+packet.sessionId);
        if (task != null) {
            task.success(packet);
        }
    }
