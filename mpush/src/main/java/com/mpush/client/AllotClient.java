 ...
 */
public final class AllotClient {
    private List<String> serverAddress = new ArrayList<>();
    public static String token;

    public static void setToken(String token) {
        AllotClient.token = token;
    }

    public AllotClient() {
    }

    public AllotClient(String toke) {
        this.token = toke;
    }

    public static AllotClient getInstance(){
        return new AllotClient();
    }


    public List<String> getServerAddress() {
        if (null != serverAddress && serverAddress.size()>0) {
    ...
    public List<String> queryServerAddressList() {
        ...
        try {
            ...
            if (config.getAllotServer().startsWith("https")) {
                ...
                ((HttpsURLConnection) connection).setRequestProperty("token", token);
            } else {
                ((HttpURLConnection) connection).setRequestProperty("token", token);
            }
