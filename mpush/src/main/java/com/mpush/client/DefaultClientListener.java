...
/*package*/ final class DefaultClientListener implements ClientListener {
    ...
    public void onReceiveRelation(Client client,String content, int messageId) {
        if (listener != null) {//dispatcher已经使用了Executor，此处直接同步调用
            listener.onReceiveRelation(client, content, messageId);
        }
    }
    ...
    public void onReceiveChat(Client client, String content, int messageId) {
        if (listener != null) {//dispatcher已经使用了Executor，此处直接同步调用
            listener.onReceiveChat(client, content, messageId);
        }
    }
    ...
    public void onReceiveGroupChat(Client client, String content, int messageId) {
        if (listener != null) {//dispatcher已经使用了Executor，此处直接同步调用
            listener.onReceiveGroupChat(client, content, messageId);
        }
    }
