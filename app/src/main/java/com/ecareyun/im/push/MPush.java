...
public final class MPush {
    ...
    public void startPush() {
        if (hasInit()) {
            Log.e("MPushEngine", "---startPush----");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                Log.e("MPushEngine", "---startForegroundService----");
                try {
                    ctx.startForegroundService(new Intent(ctx, MPushService.class));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
                Log.e("MPushEngine", "---startService----");
    ...
    public Future<Boolean> sendPush(byte[] content) {
        if (hasStarted() && client.isRunning()) {
            ...
            return client.push(PushContext.build(str));
        }
        return null;
    }
